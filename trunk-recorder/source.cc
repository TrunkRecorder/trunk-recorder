...
void Source::set_recorder_port_offset(unsigned int port, double offset_hz) {
  ...
  // sign, so flip here.
  //
  // We deliberately do NOT subtract `error` here: the `error` config value
  // is consumed by set_center_freq(center+error) on the SDR, which uses the
  // SDR's intrinsic calibration bias to land at the configured center. So
  // by the time IQ samples reach the channelizer, the SDR is already tuned
  // to `center`, and the recorder-side (center - chan) offset is correct.
  if (recorder_channelizer) {
