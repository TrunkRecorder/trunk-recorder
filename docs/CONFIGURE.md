...
| dmrRecorders     |          |               | number                      | The number of trunked-DMR voice recorders to attach to this source. One recorder is allocated per active TDMA slot, so a busy two-slot voice channel will use **two** of these. Required only for `"dmr"` systems. |
...
| type                     |    ✓     |                            | **"smartnet"**, **"p25"**, **"dmr"**, **"conventional"**, **"conventionalDMR"** or **"conventionalP25"**, **"conventionalSIGMF"** | The type of radio system. **"dmr"** is for trunked DMR (MOTOTRBO Capacity Plus / Capacity Max / Connect Plus and ETSI Tier III). |
| lcnTable                 |          |                            | object; `{ "1": 463112500, "2": 463612500, ... }`                                                                      | *For `"dmr"` systems.* Explicit map from DMR Logical Channel Number to RF frequency in Hz. Provide this if you know the LCN-to-freq assignment for the site. If you don't, supply `channels` instead and the decoder will auto-map. |
| channels (DMR)           |          |                            | array of numbers;<br />`[463112500, 463162500, 463612500]`                                                             | *For `"dmr"` systems, alternative to `lcnTable`.* List of candidate voice channel frequencies. The parser claims them on a first-come basis: the first unknown LCN seen in a grant gets the first unused freq, the next unknown LCN gets the next freq, etc. Useful when you have a band-scan list but don't know which LCN is which. The order you list freqs in is the order they'll be claimed — if a recorder lands on the wrong physical channel, reorder this list and the next session will be correct. You may supply both `lcnTable` and `channels`: `lcnTable` entries win, and `channels` is the pool for everything else. |
...
| modulation               |          | "qpsk" / "fm"              | **"qpsk"**, **"fsk4"**, **"fm"** or **"am"**                                                                            | The type of modulation that the system uses. Trunked P25 systems take `"qpsk"` (default) or `"fsk4"`. Conventional analog (`type: "conventional"`) takes `"fm"` (default) or `"am"`. Use `"am"` for civil-aviation airband and similar AM voice channels. You do not need to specify this with **conventionalDMR** systems. |
| bandwidth                |          | 12000 (FM) / 8000 (AM)     | number                                                                                                                 | *Conventional analog only* The channel bandwidth in Hz used by the prefilter. Defaults to 12 kHz for FM voice and 8 kHz for AM airband. Override when your channel spacing is unusual (e.g. 25 kHz commercial AM). |
| audioHighpass            |          | 300                        | number                                                                                                                 | *Conventional analog only* High-pass cutoff in Hz applied to the demodulated audio. The default removes CTCSS/DCS and 150 bps LSD wobble on FM. Set lower (e.g. 200) if AM audio sounds thin. |
| audioLowpass             |          | 3250 (FM) / 3000 (AM)      | number                                                                                                                 | *Conventional analog only* Low-pass cutoff in Hz applied to the demodulated audio. Defaults are tuned for voice intelligibility. |
...
#### A note on MOTOTRBO RAS (Restricted Access to System)

MOTOTRBO RAS is a proprietary feature that scrambles the CSBK CRC checksum on a trunked DMR control channel. Trunk Recorder's bundled OP25 decoder does **not** validate CSBK CRCs — see `_CRC_CHECK_ 0` in `lib/op25_repeater/lib/dmr_slot.cc` — and RAS only affects the CRC field, not the data. The opcode, FID, source/destination addresses, and channel-grant fields all extract correctly from a RAS-protected control channel without any configuration. There is no `ras` config key, and you do not need one to monitor a RAS-protected MOTOTRBO Capacity Plus / Capacity Max / Connect Plus site.
