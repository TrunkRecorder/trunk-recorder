...
public:
  ...
  int upload(Call_Data_t call_info) {
    ...
    if (curl && multi_handle) {
      ...
      if (res == CURLM_OK && easy_result == CURLE_OK && is_success_http_status(response_code)) {
        ...
        if (response_code == 202) {
          BOOST_LOG_TRIVIAL(info) << loghdr << "Rdio Scanner Upload Accepted (202) - stub cached; file size: " << file_info.st_size;
        } else {
    ...
    if (curl_errbuf[0] != '\0') {
      BOOST_LOG_TRIVIAL(error) << loghdr << "Rdio Scanner Upload Error (HTTP " << response_code << "): "
                               << response_buffer << " curl_err=" << curl_errbuf;
    } else {
      BOOST_LOG_TRIVIAL(error) << loghdr << "Rdio Scanner Upload Error (HTTP " << response_code << "): " << response_buffer;
    }
